...
Paso 2 del taller (después de tener los tokens del Paso 1). Construye el
...
[`../03-clusterctl/cluster.yaml.sample`](../03-clusterctl/cluster.yaml.sample)).
Ventaja: cambiar la versión de k8s no requiere reconstruir ningún template,
solo editar el manifiesto de `03-clusterctl/`.
...
`templateSelector.matchTags` en `../03-clusterctl/cluster.yaml.sample`.
